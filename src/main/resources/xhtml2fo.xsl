...
<xsl:template name="Trata-style">
    ...
    <xsl:for-each select="tokenize(@style, ';')">
        ...
        <xsl:if test="contains($atributo, ':')">
            ...
            <xsl:if test="matches($nome, 'text-.+|font.*|margin.*')">
...
<xsl:template match="xhtml:table">
  ...
  <fo:table><xsl:call-template name="table-common-atts"/>
...
<xsl:template match="xhtml:th|th">
  <fo:table-cell font-weight="bold" padding=".1em" border="1pt solid"><xsl:call-template name="common-atts"/>
    <xsl:if test="@colspan">
...
<xsl:template match="xhtml:td|td">
  <fo:table-cell padding=".1em" border="1pt solid"><xsl:call-template name="common-atts"/>
    <xsl:if test="@colspan">
      <xsl:attribute name="number-columns-spanned"><xsl:value-of select="@colspan"/></xsl:attribute>
    </xsl:if>
    <xsl:if test="@rowspan">
