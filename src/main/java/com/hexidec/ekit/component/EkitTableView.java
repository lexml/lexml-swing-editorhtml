 ...
 * 
 * <p>
 * Reescrita do TableView para permitir largura mínima das células. Foram
 * comentados removidos os códigos relacionados à fixação da largura das células
 * porque dependem de classes privadas do swing.
 * </p>
 * <p>
 * Código adaptado de javax.swing.text.html.TableView
 * </p>
 * 
 */
public class EkitTableView extends BoxView implements ViewFactory {

	private static final Log log = LogFactory.getLog(EkitTableView.class);

	private static final int MIN_CELL_WIDTH = 25;
	 ...
	 */
	public EkitTableView(Element elem) {
	...
	void updateInsets() {
		...
		setInsets(top, (short) painter.getInset(LEFT, this), bottom,
	...
	protected void setPropertiesFromAttributes() {
		...
		if (attr != null) {
			...
			// CSS.LengthValue lv = (CSS.LengthValue) attr
			// .getAttribute(CSS.Attribute.BORDER_SPACING);
			// if (lv != null) {
			// cellSpacing = (int) lv.getValue();
			// } else {
			// cellSpacing = 0;
			// }
			// lv = (CSS.LengthValue) attr
			// .getAttribute(CSS.Attribute.BORDER_TOP_WIDTH);
			// if (lv != null) {
		...
		if (!gridValid) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (v instanceof RowView) {
					...
					Object o = v.getElement().getAttributes()
			...
			for (int row = 0; row < nrows; row++) {
				...
				for (int cell = 0; cell < rv.getViewCount(); cell++, col++) {
					...
					if (!relativeCells) {
						...
						// CSS.LengthValue lv = (CSS.LengthValue) a
	 ...
	 * @param reqs
	 *            the requirements desired for each column. This is the column
	 *            maximum of the cells minimum, preferred, and maximum requested
	 *            span
	 * @param spans
	 *            the return value of how much to allocated to each column
	 * @param offsets
	 *            the return value of the offset from the origin for each column
	 * @return the offset from the origin and the span for each column in the
	...
	/**
	 * Calculate a tiled layout for the given iterator. This should be done
	 * collapsing the neighboring margins to be a total of the maximum of the
	 * two neighboring margin areas as described in the CSS spec.
	 */
	static void calculateTiledLayout(LayoutIterator iter, int targetSpan) {
		...
		for (int i = 0; i < n; i++) {
			...
			gain[iter.getAdjustmentWeight()] += (long) iter
					.getMaximumSpan(targetSpan) - currentPreferred;
			loss[iter.getAdjustmentWeight()] += currentPreferred
					- (long) iter.getMinimumSpan(targetSpan);
		 ...
		 * target span. This takes the margin collapsing into account prior to
		 * adjusting the span.
		 */
		...
		for (; adjustmentLevel <= LayoutIterator.WorstAdjustmentWeight; adjustmentLevel++) {
		...
		if (adjustmentLevel <= LayoutIterator.WorstAdjustmentWeight) {
			desiredAdjustment -= (adjustmentLevel > 0) ? adjustmentsArray[adjustmentLevel - 1]
					: 0;
			if (desiredAdjustment != 0) {
				float maximumAdjustment = adjustmentsArray[adjustmentLevel]
						- ((adjustmentLevel > 0) ? adjustmentsArray[adjustmentLevel - 1]
								: 0);
		...
		for (int i = 0; i < n; i++) {
			...
			if (iter.getAdjustmentWeight() < adjustmentLevel) {
				iter.setSpan((int) ((allocated > preferred) ? Math.floor(iter
						.getMaximumSpan(targetSpan)) : Math.ceil(iter
						.getMinimumSpan(targetSpan))));
			} else if (iter.getAdjustmentWeight() == adjustmentLevel) {
				int availableSpan = (allocated > preferred) ? (int) iter
						.getMaximumSpan(targetSpan) - iter.getSpan() : iter
						.getSpan() - (int) iter.getMinimumSpan(targetSpan);
				int adj = (int) Math.floor(adjustmentFactor * availableSpan);
				iter.setSpan(iter.getSpan()
						+ ((allocated > preferred) ? adj : -adj));
			}
			totalOffset = (int) Math.min(
					(long) iter.getOffset() + (long) iter.getSpan(),
					Integer.MAX_VALUE);
		}
		...
		while (roundError > 0 && canAdjust) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (roundError > 0) {
					int boundGap = (adj > 0) ? (int) Math.floor(iter
							.getMaximumSpan(targetSpan)) - curSpan : curSpan
							- (int) Math.ceil(iter.getMinimumSpan(targetSpan));
					if (boundGap >= 1) {
						canAdjust = true;
	...
	void checkMultiColumnCell(int axis, int col, int ncols, View v) {
		...
		if (cmin > min) {
			 ...
			 * table cell.... calculate the adjustments.
			 */
	 ...
	 * requirements of the columns and then adjust it if the CSS width or height
	 * attribute is specified and applicable to the axis.
	 */
	protected SizeRequirements calculateMinorAxisRequirements(int axis,
		...
		// CSS.LengthValue cssWidth = (CSS.LengthValue) attr
		// .getAttribute(CSS.Attribute.WIDTH);
		//
		// if (spanSetFromAttributes(axis, r, cssWidth, null)) {
	...
	/**
	 * Adjust the given requirements to the CSS width or height if it is
	 * specified along the applicable axis. Return true if the size is exactly
	 * specified, false if the span is not specified in an attribute or the size
	 * specified is a percentage.
	 */
	// static boolean spanSetFromAttributes(int axis, SizeRequirements r,
	// CSS.LengthValue cssWidth, CSS.LengthValue cssHeight) {
	// if (axis == X_AXIS) {
	...
	// r.minimum = r.preferred = r.maximum = (int) cssHeight
	// .getValue();
	// return true;
	// }
	// }
	...
	/**
	 * Calculate the requirements for the major axis. This is called by the
	 * superclass whenever the requirements need to be updated (i.e. a
	 * preferenceChanged was messaged through this view).
	 * <p>
	...
	/**
	 * Calculate the requirements needed to tile the requirements given by the
	 * iterator that would be tiled. The calculation takes into consideration
	 * margin and border spacing.
	 */
	private static SizeRequirements calculateTiledRequirements(
			LayoutIterator iter, SizeRequirements r) {
		...
		r.minimum = (minimum > Integer.MAX_VALUE) ? Integer.MAX_VALUE
				: (int) minimum;
		r.preferred = (preferred > Integer.MAX_VALUE) ? Integer.MAX_VALUE
				: (int) preferred;
		r.maximum = (maximum > Integer.MAX_VALUE) ? Integer.MAX_VALUE
				: (int) maximum;
		return r;
	}
	 ...
	 * needs to be updated along the minor axis.
	 * <p>
	 * This is implemented to call the <a
	 * href="#layoutColumns">layoutColumns</a> method, and then forward to the
	 ...
	 * arrays which represent the allocations to the children along the minor
	 * axis. This is called by the superclass whenever the layout needs to be
	 * updated along the minor axis.
	 * <p>
	 * This method is where the layout of the table rows within the table takes
	 * place. This method is implemented to call the use the RowIterator and the
	 * CSS collapsing tile to layout with border spacing and border collapsing
	 * capabilities.
	 * 
	 * @param targetSpan
	 *            the total span given to the view, which whould be used to
	 ...
	 * @return the view representing the given position, or null if there isn't
	 *         one
	 */
	 ...
	 * is implemented to delegate to the css box painter to paint the border and
	 * background prior to the interior. The superclass culls rendering the
	 * children that don't directly intersect the clip and the row may have
	 * cells hanging from a row above in it. The table does not use the
	 * superclass rendering behavior and instead paints all of the rows and lets
	 * the rows cull those cells not intersecting the clip region.
	 * 
	 * @param g
	...
	/**
	 * Fetches the ViewFactory implementation that is feeding the view
	 * hierarchy. This replaces the ViewFactory with an implementation that
	 * calls through to the createTableRow and createTableCell methods. If the
	 * element given to the factory isn't a table row or cell, the request is
	 * delegated to the factory produced by the superclass behavior.
	 * 
	 ...
	 * location that this view is responsible for. This replaces the ViewFactory
	 * with an implementation that calls through to the createTableRow and
	 * createTableCell methods. If the element given to the factory isn't a
	 * table row or cell, the request is delegated to the factory passed as an
	 * argument.
	 * 
	 * @param e
	...
	/**
	 * The index of the caption view if there is a caption. This has a value of
	 * -1 if there is no caption. The caption lives in the inset area of the
	 * table, and is updated with each time the grid is recalculated.
	 */
	private int captionIndex;

	/**
	 * Do any of the table cells contain a relative size specification? This is
	 * updated with each call to updateGrid(). If this is true, the
	 * ColumnIterator will do extra work to calculate relative cell
	 * specifications.
	 */
	private boolean relativeCells;

	/**
	 * Do any of the table cells span multiple rows? If true, the
	 * RowRequirementIterator will do additional work to adjust the requirements
	 * of rows spanned by a single table cell. This is updated with each call to
	 * updateGrid().
	 */
	...
	class ColumnIterator implements LayoutIterator {
		...
		/**
		 * Disable percentage adjustments which should only apply when
		 * calculating layout, not requirements.
		 */
		...
		private void updatePercentagesAndAdjustmentWeights(int span) {
			...
			for (int rowIndex = 0; rowIndex < nrows; rowIndex++) {
				...
				for (int cell = 0; cell < ncells; cell++, col++) {
					...
					// percentages[col + i] = Math.max(percentages[col
					// + i], len);
	...
	interface LayoutIterator {
		...
		int getAdjustmentWeight(); // 0 is the best weight WorstAdjustmentWeight
									// is a worst one
	...
	class RowIterator implements LayoutIterator {
		...
		public float getPreferredSpan(float parentSpan) {
			...
			if (rv != null) {
				...
				return rv.getPreferredSpan(EkitTableView.this.getAxis())
						+ adjust;
			}
			return 0;
		}
	...
	public class RowView extends BoxView {
		...
		public void preferenceChanged(View child, boolean width, boolean height) {
			...
			if (EkitTableView.this.multiRowCells && height) {
				for (int i = rowIndex - 1; i >= 0; i--) {
					RowView rv = EkitTableView.this.getRow(i);
					if (rv.multiRowCells) {
		 ...
		 * This is implemented to delegate to the css box painter to paint the
	...
	/**
	 * Default view of an html table cell. This needs to be moved somewhere
	 * else.
	 */
	class CellView extends BlockView {
		 ...
		 * represents). The results of the layout should be placed in the given
		 * arrays which represent the allocations to the children along the
		 * major axis. This is called by the superclass to recalculate the
		 * positions of the child views when the layout might have changed.
		 * <p>
		 * This is implemented to delegate to the superclass to tile the
		 * children. If the target span is greater than was needed, the offsets
		 * are adjusted to align the children (i.e. position according to the
		...
		protected void layoutMajorAxis(int targetSpan, int axis, int[] offsets,
			...
			if (used < targetSpan) {
				...
				String valign = (String) getElement().getAttributes()
						.getAttribute(HTML.Attribute.VALIGN);
				if (valign == null) {
					AttributeSet rowAttr = getElement().getParentElement()
							.getAttributes();
					valign = (String) rowAttr
							.getAttribute(HTML.Attribute.VALIGN);
				}
		...
		/**
		 * Calculate the requirements needed along the major axis. This is
		 * called by the superclass whenever the requirements need to be updated
		 * (i.e. a preferenceChanged was messaged through this view).
		 * <p>
		 * This is implemented to delegate to the superclass, but indicate the
		 * maximum size is very large (i.e. the cell is willing to expend to
		 * occupy the full height of the row).
		 * 
		 * @param axis
		 *            the axis being layed out.
		 * @param r
		...
		protected SizeRequirements calculateMinorAxisRequirements(int axis,
			...
			// rv.minimum = Math.max(Math.min(rv.minimum, min), MIN_CELL_WIDTH);
		...
		public float getPreferredSpan(int axis) {
			...
			if (axis == X_AXIS) {
				span = Math.max(span, MIN_CELL_WIDTH);
			}
			return span;
		}
