...
public class FormatAction extends StyledEditorKit.StyledTextAction {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (selText == null || textLength < 1) {
			...
			parentTextPane.moveCaretPosition(caretOffset);
			
			SimpleAttributeSet sasText = new SimpleAttributeSet(
					parentTextPane.getCharacterAttributes());
			
			boolean replace;
			SimpleAttributeSet sasEmpty = new SimpleAttributeSet();
			if(sasText.containsAttribute(htmlTag, sasEmpty)) {
				sasText.removeAttribute(htmlTag);
				replace = true;
			}
			else {
				sasText.addAttribute(htmlTag, sasEmpty);
				replace = false;
			}
			
			parentTextPane.select(caretOffset, caretOffset + textLength);
			parentTextPane.setCharacterAttributes(sasText, replace);
